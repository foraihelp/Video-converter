...
Desktop app (Windows/macOS) for batch re-encoding video files into `.mov` with selectable codecs.
...
## Supported output codecs (all within the `.mov` container)
...
Alpha channel, audio track handling (copy/PCM/AAC/none), metadata, and timecode preservation are all configurable per conversion.
